...
def general_parse(code):
    global frequencies, lambda_var_counters
    ...
    lambda_var_counters = {i: [] for i in lambda_vars.keys()}
...
def parse(code, spacing="\n "):
    ...
    if active_char in ".123456789":
        return num_parse(active_char, rest_code, frequencies)
    # String literals
    if active_char == '"':
        return str_parse(active_char, rest_code, frequencies)
    ...
    if active_char in variables:
        if active_char in frequencies:
            if any(active_char in v for i,v in lambda_var_counters.items()):
                frequencies[active_char][1] += 1
            else:
                frequencies[active_char][0] += 1
        return active_char, rest_code
    # Replace replaements
...
def lambda_function_parse(active_char, rest_code):
    ...
    if "," in var:
        sp_var = var.split(", ")
        lambda_var_counters[active_char]+=sp_var
    else:
        lambda_var_counters[active_char]+=[var]
    # Take one argument, the lambda.
    parsed, rest_code = parse(rest_code)
    args_list = [parsed]
    # Rotate back.
    lambda_vars[active_char] = saved_lambda_vars
    lambda_var_counters[active_char] = lambda_var_counters[active_char][:-2] if "," in var else lambda_var_counters[active_char][:-1]
...
def replace_parse(active_char, rest_code, spacing):
    ...
    if active_char == "\\":
        ...
        lambda_var_counters["u"] += ["G", "H"]
    elif active_char == "L":
        lambda_var_counters["u"] += ["b"]
